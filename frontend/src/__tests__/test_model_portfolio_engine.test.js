...
describe('Model Portfolio & Multi-Horizon Engine', () => {
    ...
    it('buildModelPortfolio sizes shares and caps sector exposure correctly', () => {
        const ranked = analyzeTopNextDayPicks(mockTrades, { horizonStyle: 'swing_7_21d' });
        const portfolio = buildModelPortfolio(ranked, { totalCapital: 500000, maxPositions: 4, maxSectorExposurePct: 40 });

        expect(portfolio.positions.length).toBeGreaterThan(0);
        expect(portfolio.metrics.totalInvested).toBeLessThanOrEqual(500000);
